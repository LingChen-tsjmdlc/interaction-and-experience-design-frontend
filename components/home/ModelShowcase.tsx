...
function ModelCardItem({ model, tagColors, index }: {
    ...
    >
      ...
      <div className="absolute top-0 right-0 w-20 h-20 opacity-0 group-hover/feature:opacity-100 transition-opacity duration-500 pointer-events-none">
        <div className={cn(
          ...
          model.tags.includes("inference") && !model.tags.includes("text") && "bg-orange-400/15",
        )} />
      </div>
...
export function ModelShowcase() {
  ...
  const modelData = useMemo(() => {
    try {
      const models: ModelCard[] = t("modelShowcase.models", { returnObjects: true }) as unknown as ModelCard[];
      return models;
    } catch {
      return [];
    }
  ...
  const tagColors = useMemo(() => {
    try {
      const colors = t("modelShowcase.tagColors", { returnObjects: true }) as Record<string, string>;
      return colors;
    } catch {
      return {};
    }
